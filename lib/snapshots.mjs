...
export async function loadCase(caseId, root = process.cwd()) {
  const id = safeCaseId(caseId);
  const file = path.join(root, 'cases', `${id}.json`);
...
export function caseRoot(caseId, root = process.cwd()) {
  return path.join(root, 'research', 'cases', safeCaseId(caseId));
}

export async function ensureCaseMetadata(caseConfig, root = process.cwd()) {
...
export async function listSnapshotManifests(caseId, root = process.cwd()) {
