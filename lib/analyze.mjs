...
function classifyRow(row, preset, options) {
  ...
  const sourceText = `${phrase} ${(row.seeds || []).join(' ')}`;
  ...
  for (const intent of preset.intents || []) {
    ...
    for (const keyword of intent.keywords || []) {
      if (matchesKeyword(sourceText, keyword)) {
...
export function analyzeRows(rows, preset, options = {}) {
  ...
  return {
    meta: {
      ...
      note: 'maxCount и relativeDemandBand — сигналы внутри текущей выборки, а не суммарный поисковый объём.',
      ...stats,
    },
    summary,
    assignedRows,
